...
This functions executes a count query on ``collection``. In contrast with :ref:`mongoc_collection_count_documents()`, the count returned is *not* guaranteed to be accurate.
...
This method is implemented using the `count <https://www.mongodb.com/docs/manual/reference/command/count/>`_ command. Due to an oversight in versions 5.0.0-5.0.8 of MongoDB, the ``count`` command was not included in version "1" of the Stable API. Applications using this method with the Stable API are recommended to upgrade their server version to 5.0.9+ or disable strict mode (via `:ref:`mongoc_server_api_strict()`) to avoid encountering errors.
